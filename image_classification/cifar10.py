...
def inference1(images):
    ...
    return logits
def inferencemoe(images):
    images=tf.layers.flatten(images)
 
    gate_weight=slim.fully_connected(
         images, 20, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    gate_distribution=tf.nn.softmax(tf.reshape(gate_weight,[-1,2]))
    gate1=tf.reshape(gate_distribution[:,0],[-1,10])
    gate2=tf.reshape(gate_distribution[:,1],[-1,10])
    images_reshape=slim.fully_connected(
         images, 512, activation_fn=tf.nn.relu,
        weights_regularizer=slim.l2_regularizer(1e-8))
    ...
    expert_activations=slim.fully_connected(
         images_reshape, NUM_CLASS*2, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    expert_distribution = tf.nn.relu(
        tf.reshape(expert_activations,
                   [-1, 2]))
    output1=tf.reshape(expert_distribution[:,0],[-1,10])
    output2=tf.reshape(expert_distribution[:,1],[-1,10])
    gate_output1=tf.multiply(output1,gate1)
    gate_output2=tf.multiply(output2,gate2)    
    # Define summery-operation for 'logits'-variable
    logits=tf.add(gate_output1,gate_output2)
    _activation_summary(logits)

    return logits
def inference2(images):
    image_gate=tf.layers.flatten(images)
    gate_weight=slim.fully_connected(
         image_gate, 20, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    gate_distribution=tf.nn.softmax(tf.reshape(gate_weight,[-1,2]))
    gate1=tf.reshape(gate_distribution[:,0],[-1,10])
    gate2=tf.reshape(gate_distribution[:,1],[-1,10])
    ...
    images_reshape=tf.layers.flatten(images)

    images_reshape=slim.fully_connected(
         images_reshape, 512, activation_fn=tf.nn.relu,
        weights_regularizer=slim.l2_regularizer(1e-8))
    ...
    gate_output1=tf.multiply(softmax_linear,gate1)
    gate_output2=tf.multiply(output2,gate2)
    logits=tf.add(gate_output1,gate_output2)
    _activation_summary(logits)
...
def inference_rnn(images):
    inputs=tf.reshape(images,[-1, 24, 72])
    stacked_GRU = tf.contrib.rnn.MultiRNNCell(
            [
                tf.contrib.rnn.GRUCell(128)
                for _ in range(2)
    ...
    with tf.variable_scope("RNN"):
      outputs, state = tf.nn.dynamic_rnn(stacked_GRU, inputs,
                                         ...
                                         dtype=tf.float32)
    final_output = slim.fully_connected(
        state, 10, activation_fn=tf.nn.relu,
        weights_regularizer=slim.l2_regularizer(1e-8))
...
def inference3(images):
    ...
    gate_output1=tf.multiply(softmax_linear,gate1)
    gate_output2=tf.multiply(final_output,gate2)
    logits=tf.add(gate_output1,gate_output2)
    _activation_summary(logits)
    
    return logits

def inferencednn_rnn(images):
    image_gate=tf.layers.flatten(images)
    gate_weight=slim.fully_connected(
         image_gate, 170, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    gate_weight=tf.nn.dropout(gate_weight,rate=0.05)
    gate_weight=slim.fully_connected(
         gate_weight, 20, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    gate_distribution=tf.nn.softmax(tf.reshape(gate_weight,[-1,2]))
    gate1=tf.reshape(gate_distribution[:,0],[-1,10])
    gate2=tf.reshape(gate_distribution[:,1],[-1,10])
    ...
    gate_output1=tf.multiply(output2,gate1)
    gate_output2=tf.multiply(final_output,gate2)
    logits=tf.add(gate_output1,gate_output2)
    _activation_summary(logits)
    
    return logits
def inferencednn_rnn1(images):
    image_gate=tf.layers.flatten(images)
    gate_weight=slim.fully_connected(
         image_gate, 20, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
...
def inference3model(images):
    image_gate=tf.layers.flatten(images)

    gate_weight=slim.fully_connected(
         image_gate, 30, activation_fn=None,
        weights_regularizer=slim.l2_regularizer(1e-8))
    gate_distribution=tf.nn.softmax(tf.reshape(gate_weight,[-1,3]))
    gate1=tf.reshape(gate_distribution[:,0],[-1,10])
    gate2=tf.reshape(gate_distribution[:,1],[-1,10])
    gate3=tf.reshape(gate_distribution[:,2],[-1,10])
    # conv1
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay('weight', shape=[5, 5, 3, 64], stddev=5e-2, wd=None)
        conv = tf.nn.conv2d(images, kernel, strides=[1, 1, 1, 1], padding='SAME')
    ...
    gate_output1=tf.multiply(softmax_linear,gate1)
    gate_output2=tf.multiply(final_output,gate2)
    gate_output3=tf.multiply(output3,gate3)
    logits=tf.add(gate_output1,gate_output2)
    logits=tf.add(logits,gate_output3)
    _activation_summary(logits)
