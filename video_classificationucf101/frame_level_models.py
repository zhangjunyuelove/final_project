...
flags.DEFINE_integer("fv_cluster_size", 32,
                     "Number of units in the NetVLAD cluster layer.")
...
class GatedMoe2(models.BaseModel):
  ...
  def create_model(self,
    ...
    mooegating_distribution = tf.nn.softmax(tf.reshape(
        moegate_activations,
        [-1, num_model ]))  # (Batch * #Labels) x (num_model + 1)
...
class GatedMoe_poolingmlp(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    prediction_model_weight2=tf.multiply(image_activation,moe_gates_model2)    
    prediction_final=tf.add(prediction_model_weight1,prediction_model_weight2)
    return {"predictions": prediction_final}

class GatedMoe_poolingmlp1(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    reshape_input1=tf.reshape(model_input1, [-1, 10*feature_size])
...
class MLP(models.BaseModel):
  ...
  def create_model(self,
    ...
    image_activation = slim.fully_connected(
        image_input, vocab_size, activation_fn=tf.nn.sigmoid,
        weights_regularizer=slim.l2_regularizer(1e-8))   
    return {"predictions": image_activation}

class MLP_deep(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    image_input=tf.reshape(reshape_input2[:,0,:],[-1,feature_size])
    image_activation = slim.fully_connected(
        image_input, 500, activation_fn=tf.nn.relu,
        weights_regularizer=slim.l2_regularizer(1e-8))   
    image_activation = slim.fully_connected(
        image_activation, vocab_size, activation_fn=tf.nn.sigmoid,
        weights_regularizer=slim.l2_regularizer(1e-8)) 
    return {"predictions": image_activation}
class Conv2d(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    max_frames1 = model_input.get_shape().as_list()[1]
    feature_size = model_input.get_shape().as_list()[2]
    feature_size1=tf.cast(feature_size,tf.float32)
    reshape_input2=tf.reshape(model_input, [-1, max_frames1,feature_size])
    image_input=tf.reshape(reshape_input2[:,0,:],[-1, tf.sqrt(feature_size1), tf.sqrt(feature_size1),1])
    filter1=tf.get_variable("filter1",
          [5,5,1,32],
          initializer = tf.random_normal_initializer(stddev=0.1))

    
    conv2d1 = tf.nn.conv2d(image_input, filter1, strides=[1, 1, 1, 1], padding='SAME')
    pool1=tf.nn.max_pool(conv2d1, ksize=[1, 2, 2, 1],
                          strides=[1, 2, 2, 1], padding='SAME')
    filter2=tf.get_variable("filter2",
          [5, 5, 32, 64],
          initializer = tf.random_normal_initializer(stddev=0.1))
    
    conv2d2 = tf.nn.conv2d(pool1, filter2, strides=[1, 1, 1, 1], padding='SAME')
    pool2=tf.nn.max_pool(conv2d2, ksize=[1, 2, 2, 1],
                          strides=[1, 2, 2, 1], padding='SAME')

    
    
    flaten=tf.layers.flatten(pool2)
    image_activation = slim.fully_connected(
        flaten, 1024, activation_fn=tf.nn.relu,
        weights_regularizer=slim.l2_regularizer(1e-8)) 
    image_activation = slim.fully_connected(
        image_activation, vocab_size, activation_fn=tf.nn.sigmoid,
        weights_regularizer=slim.l2_regularizer(1e-8))
...
class Conv3d(models.BaseModel):
  ...
  def create_model(self,
    ...
    feature_size = model_input.get_shape().as_list()[2]
    feature_size1=tf.cast(feature_size,tf.float32)
    reshape_input=tf.reshape(model_input, [-1, iterations,tf.sqrt(feature_size1), tf.sqrt(feature_size1),1])
    ...
    conv3d1 = tf.nn.conv3d(reshape_input, filter1, strides=[1, 1,1, 1, 1], padding='SAME')
    pool1=tf.nn.max_pool3d(conv3d1, ksize=[1,3, 3, 3, 1],
                          strides=[1,2, 2, 2, 1], padding='SAME')


    
    
    flaten=tf.layers.flatten(pool1)

    image_activation = slim.fully_connected(
        flaten, vocab_size, activation_fn=tf.nn.sigmoid,
        weights_regularizer=slim.l2_regularizer(1e-8))
...
class GatedMoe3m(models.BaseModel):
  ...
  def create_model(self,
    ...
    return {"predictions": prediction_final}
class GatedMoe3m1(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    prediction_final=tf.add(prediction_final1,prediction_model_weight3)
    return {"predictions": prediction_final}
class GatedMoe4m(models.BaseModel):
  """Creates a NetVLAD based model.
  ...
  def create_model(self,
    ...
    reshape_input1=tf.reshape(model_input1, [-1, 20*feature_size])
    


    moegate_activations = slim.fully_connected(
        reshape_input1,
        vocab_size * (num_model+2 ),
        activation_fn=None,
        biases_initializer=None,
        weights_regularizer=slim.l2_regularizer(l2_penalty),
        scope="moegates1")      
    ...
    mooegating_distribution = tf.nn.softmax(tf.reshape(
        moegate_activations,
        [-1, num_model + 2]))  # (Batch * #Labels) x (num_model + 1)
    moe_gates_model1=tf.reshape(mooegating_distribution[:,0],[-1,vocab_size])
    moe_gates_model2=tf.reshape(mooegating_distribution[:,1],[-1,vocab_size])
    moe_gates_model3=tf.reshape(mooegating_distribution[:,2],[-1,vocab_size])
    moe_gates_model4=tf.reshape(mooegating_distribution[:,3],[-1,vocab_size])
    ...
    prediction_model4=aggregated_model().create_model(
    ...
    prediction_model_weight4=tf.multiply(prediction_model4,moe_gates_model4)  

    prediction_final=tf.add(prediction_model_weight1,prediction_model_weight2)
    prediction_final=tf.add(prediction_final,prediction_model_weight3)
    prediction_final=tf.add(prediction_final,prediction_model_weight4)
    return {"predictions": prediction_final}