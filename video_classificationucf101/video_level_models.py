...
class MoeModel1(models.BaseModel):
  """A softmax over a mixture of logistic models (with L2 regularization)."""
...
class MoeModel2(models.BaseModel):
  ...
  def create_model(self,
    ...
    if gating_probabilities:
        if gating_input == 'prob':
            gating_weights = tf.get_variable("gating_prob_weights1",
              [vocab_size, vocab_size],
              initializer = tf.random_normal_initializer(stddev=1 / math.sqrt(vocab_size)))
            gates = tf.matmul(probabilities, gating_weights)
        else:
...
class MoeModel3(models.BaseModel):
  """A softmax over a mixture of logistic models (with L2 regularization)."""
  ...
  def create_model(self,
    ...
    if gating_probabilities:
        if gating_input == 'prob':
            gating_weights = tf.get_variable("gating_prob_weights31",
              [vocab_size, vocab_size],
              initializer = tf.random_normal_initializer(stddev=1 / math.sqrt(vocab_size)))
            gates = tf.matmul(probabilities, gating_weights)
        else:
