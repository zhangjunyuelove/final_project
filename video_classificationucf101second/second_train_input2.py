...
if __name__ == '__main__':
  flags.DEFINE_string("train_dir", "E:/Youtube-8M-WILLOW-master/model/vlad_model_2",
                      "The directory to load the model files from.")
  flags.DEFINE_string("output_file", "predictions_11.csv",
                      "The file to save the predictions to.")
  flags.DEFINE_string(
      "input_data_pattern", "E:/UCF101_TRAIN/ucf_train_first.tfrecord",
...
def format_lines( video_ids,predictions, top_k,label_val):
  ...
  for video_index in range(batch_size):
    ...
    yield ",".join(" %f" % pair for pair in line)+"\n"
  ...
  with tf.Session() as sess, gfile.Open(out_file_location, "w+") as out_file:
    ...
    try:
      while not coord.should_stop():
          ...
          logging.info('------------------final performance----------------'+("%.2f" % hit_at_one))
          for line in format_lines(video_id_batch_val, predictions_val, top_k,label_val):
            out_file.write(line)
          out_file.flush()
        ...
        logging.info('------------------final performance----------------'+("%.2f" % accuracy_performance))
    finally:
        coord.request_stop()
