...
if __name__ == "__main__":
  # Dataset flags.
  flags.DEFINE_string("train_dir", "E:/Moe_Junyue/moe_model/111",
                      "The directory to save the model files in.")
  flags.DEFINE_string("model_feature_path", "E:/Moe_Junyue/dataset/combined.tfrecords", "second_train_input.")

  flags.DEFINE_string(
      "train_data_pattern", "E:/UCF101_TRAIN/ucf_test.tfrecord",
  ...
  flags.DEFINE_float("learning_rate_decay_examples", 30000,
...
def parse_csv(line):
    ...
    model_input_raw=tf.reshape(parsed_line[101:1125],shape=(1024,))
    model1_input=tf.reshape(parsed_line[1125:1226],shape=(101,))
    model2_input=tf.reshape(parsed_line[1226:],shape=(101,))
    batch_labels = tf.expand_dims(label, 0)
    batch_model_input_raw = tf.expand_dims(model_input_raw, 0)
    batch_model1_input = tf.expand_dims(model1_input, 0)
    batch_model2_input = tf.expand_dims(model2_input, 0)
    d=batch_labels,batch_model_input_raw,batch_model1_input,batch_model2_input
    return d
def decode(serialized_example):
  ...
  return features['labels_batch'],features['model_input_raw'],features['model1_input'], features['model2_input']
def build_graph(reader,
  ...
  with tf.name_scope("train_input"):
      filename = [model1_path]
      dataset = tf.data.TFRecordDataset(filename).map(decode)
      dataset_shuffle = dataset.shuffle(buffer_size=10000)
      batch_model_1=dataset_shuffle.batch(batch_size)
      train_dataset_epoch = batch_model_1.repeat(num_epochs)
      iterator = train_dataset_epoch.make_one_shot_iterator()
      labels_batch,model_input_raw,model1_input,model2_input = iterator.get_next()
  ...
  model1_input=tf.nn.l2_normalize(model1_input, feature_dim)
  model2_input=tf.nn.l2_normalize(model2_input, feature_dim)
  ...
  with tf.name_scope("model"):
    ...
    tf.add_to_collection("input_batch1", model1_input)
    tf.add_to_collection("input_batch2", model2_input)
...
class Trainer(object):
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          if self.is_master:
            ...
            if self.is_master and time_to_export:
              saver.save(sess, "E:/Moe_Junyue/moe_model/overall/model.ckpt", global_step_val)
  ...
  def build_model(self, model, reader):
    ...
    build_graph(reader=reader,
                 model=model,
                 model1_path=FLAGS.model_feature_path,
...
def get_reader():
  ...
  if FLAGS.frame_features:
    reader = reader2.YT8MFrameFeatureReader(
        feature_names=feature_names, feature_sizes=feature_sizes)
  else:
    reader = reader2.YT8MAggregatedFeatureReader(
        feature_names=feature_names, feature_sizes=feature_sizes)
