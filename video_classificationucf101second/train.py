...
if __name__ == "__main__":
  # Dataset flags.
  flags.DEFINE_string("train_dir", "E:/Moe_Junyue/moe",
...
def build_graph(reader,
  ...
  with tf.name_scope("model"):
    ...
    tf.add_to_collection('model_input1', model_input1)
  ...
  def run(self, start_new_model=False):
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        ...
        model_input1=tf.get_collection("model_input1")[0]
    ...
    with sv.managed_session(target, config=self.config) as sess:
      ...
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          _, global_step_val, loss_val, predictions_val, labels_val,model_input_batch = sess.run(
              [train_op, global_step, loss, predictions, labels,model_input1])
