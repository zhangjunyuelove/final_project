...
if __name__ == '__main__':
  flags.DEFINE_string("train_dir", "E:/Moe_Junyue/moe_model/overall",
                      "The directory to load the model files from.")
  flags.DEFINE_string("output_file", "predictions_test_overall.csv",
                      "The file to save the predictions to.")
  flags.DEFINE_string(
      "input_data_pattern",  "E:/Moe_Junyue/dataset/combined_test.tfrecords",
      "File glob defining the evaluation dataset in tensorflow.SequenceExample "
      "format. The SequenceExamples are expected to have an 'rgb' byte array "
      "sequence feature as well as a 'labels' int64 context feature.")
  flags.DEFINE_string(
      "input_data_pattern2",  "E:/UCF101_TRAIN/ucf_test.tfrecord",
      "File glob defining the evaluation dataset in tensorflow.SequenceExample "
      "format. The SequenceExamples are expected to have an 'rgb' byte array "
      "sequence feature as well as a 'labels' int64 context feature.")
  # Model flags.
...
def get_input_data_tensors(data_pattern,batch_size):
    with tf.name_scope("input"):
       ...
       return labels_batch, model_input_raw, model1_input, model2_input

def get_input_id_tensors(reader, data_pattern, batch_size, num_readers=1):
...
def format_lines(video_ids, predictions, top_k, labels):
  ...
  for video_index in range(batch_size):
    label_actual= [np.argmax(one_hot)for one_hot in labels]
    ...
    yield video_ids[video_index].decode('utf-8') + "," + " ".join("%i %f" % pair
                                                  for pair in line) +","+ str(label_actual[video_index])+"\n"
                   
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k,data_pattern2):
  with tf.Session() as sess, gfile.Open(out_file_location, "w+") as out_file:
    labels_batch, model_input_raw, model1_input, model2_input = get_input_data_tensors( data_pattern, batch_size)
    video_id_batch=get_input_id_tensors(reader, data_pattern2, batch_size)
    ...
    try:
      while not coord.should_stop():
          labels_batch_val, model_input_raw_val, model1_input_val, model2_input_val,video_id_batch_val = sess.run([labels_batch, model_input_raw, model1_input, model2_input,video_id_batch])
          predictions_val, = sess.run([predictions_tensor], feed_dict={input_tensor: model_input_raw_val, input_tensor1: model1_input_val,input_tensor2: model2_input_val})
          now = time.time()
...
def main(unused_argv):
  ...
  inference(reader, FLAGS.train_dir, FLAGS.input_data_pattern,
    FLAGS.output_file, FLAGS.batch_size, FLAGS.top_k,FLAGS.input_data_pattern2)
